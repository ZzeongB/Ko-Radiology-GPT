...
def parse_args():
    ...
    parser.add_argument("--fine_tuned_model_dir", type=str, required=True, help="Path to the Ko-Radiology-GPT model directory")
...
def main(input_path, save_path, fine_tuned_model_dir):
    # Load data from CSV
    ...
    fine_tuned_model = PeftModel.from_pretrained(base_model, fine_tuned_model_dir)
    ...
    for index, row in df.iterrows():
        ...
        response = generate_text(fine_tuned_model, tokenizer, prompt)
        ...
        df.at[index, 'koGPT_answer'] = answer
...
if __name__ == "__main__":
    args = parse_args()
    main(args.input_path, args.save_path, args.fine_tuned_model_dir)
